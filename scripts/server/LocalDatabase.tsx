import { defaultProfile, accountsData, bannerData } from "./DatabaseStorage";
import { ErrorType } from "./Error";

let localAccData;
if (!localAccData) {
  localAccData = accountsData.slice(0, accountsData.length);
}

function getAccount(username: string, phoneNumber?: string): GenericObjectType {
  let acc = undefined;
  localAccData.forEach((data) => {
    if (
...
export function requestBanners(callback: APIReturnFunction): void {
  callback({ b: bannerData }, undefined);
}
